...
public class VideoProfile {
    ...
    public void copyToMediaRecorder(MediaRecorder media_recorder) {
        ...
        // also match order of MediaRecorder.setProfile() just to be safe, see OnePlusCamera/5524672/is-it-possible-to-use-camcorderprofile-without-audio-source
        media_recorder.setOutputFormat(this.fileFormat);
        media_recorder.setVideoFrameRate(this.videoFrameRate);
        // it's probably safe to always call setCaptureRate, but to be safe (and keep compatibility with old OnePlus Camera versions), we only do so when needed
