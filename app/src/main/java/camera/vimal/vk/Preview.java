...
public class Preview implements SurfaceHolder.Callback, TextureView.SurfaceTextureListener {
	...
	private boolean is_test; // whether called from OnePlusCamera.test testing
	 ...
	 */
	public void retryOnePlusCamera() {
		if( MyDebug.LOG )
			Log.d(TAG, "retryOnePlusCamera()");
        if( camera_controller == null ) {
    		if( MyDebug.LOG )
	...
	public CameraController.Size getOptimalPreviewSize(List<CameraController.Size> sizes) {
		...
		Activity activity = (Activity)this.getContext();
        {
			...
			// (a) it would be good to not assume OnePlus Camera runs in landscape mode (if we ever ran in portrait mode,
		...
		activity.runOnUiThread(new Runnable() {
			public void run() {
				...
				if( !((Activity)getContext()).isFinishing() ) {
					...
					// OnePlusCamera/47548317/what-belong-is-badtokenexception-at-classes-of-project and
